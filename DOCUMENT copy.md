...
3. Стажер 3 business_logic.py
создать файл business_logic.py
создать функции:
def get_order_by_status(storage: OrderStorage, status) - 
вернуть всех заказы с определенным статусом
def revenue_today (storage: OrderStorage) - Общая выручка, только выполненных заказов
def pending_revenue(storage: OrderStorage) - сумма цен всех заказов

4. Стажер 4

Общее приложение создать файл flowers_cli.py
1. новый заказ
2. Статистика
3. Активный заказы
4. Обзор по статусам
